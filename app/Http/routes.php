...
Route::get('session-get', function(SammyK\LaravelFacebookSdk\LaravelFacebookSdk $fb){
    ...
    return view('data-return')->with(compact('response'));
});


Route::get('profile', function(SammyK\LaravelFacebookSdk\LaravelFacebookSdk $fb){
    $token = Session::get('fb_user_access_token');
    $fb->setDefaultAccessToken((string) $token);
    
    $response = $fb->get('/me/picture?fields=url');

    return view('profile')->with(compact('response'));
});
